  ...
  $.fn.joyride = function(options) {
    ...
    return this.each(function() {
      ...
      tipTemplate = function(tipClass, index, buttonText, self) {
          var move = 0;if ($(self).data('move')) { move = $(self).data('move');}
          return '<div class="joyride-tip-guide ' +
        tipClass + '" id="joyRidePopup' + index + '" data-move="'+ move +'"><span class="joyride-nub"></span>' +
        ...
        if (count < tipContent.length) {
          ...
          if (settings.tipLocation == "bottom") {
            currentTip.offset({top: (currentTipPosition.top + currentParentHeight + nubHeight),
              left: (currentTipPosition.left - bodyOffset.left) - $(currentTip).data('move')});
